...
from flask import Flask, render_template_string, request, redirect, url_for, Response, flash, session, Blueprint
...
def hdr(user):
    return '<div class="header"><h1>Data Hub Pro v3.3</h1><div style="display:flex;align-items:center;gap:12px;"><button id="themeBtn" class="theme-toggle" onclick="toggleTheme()">☀️</button><span style="color:var(--text2)">' + user + '</span><a href="/datahub/logout" class="btn btn-red btn-sm">Logout</a></div></div><div class="container"><div class="nav"><a href="/datahub/">Data</a> <a href="/datahub/overview">Overview</a> <a href="/datahub/dashboard">Dashboard</a> <a href="/datahub/pivot">Pivot</a> <a href="/datahub/compare">Compare</a> <a href="/datahub/templates">Templates</a> <a href="/datahub/logs">Logs</a> <a href="/datahub/backup" class="btn btn-orange btn-sm">Backup</a></div>'

# ==================== LOGIN ====================
LOGIN_HTML = """<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Data Hub Pro v3.3</title><style>body{font-family:sans-serif;background:#0d1117;color:#e6edf3;display:flex;justify-content:center;align-items:center;height:100vh;margin:0}.box{background:#161b22;border:1px solid #30363d;border-radius:8px;padding:24px;width:400px}h3{color:#58a6ff;margin-bottom:16px}input{width:100%;padding:10px;background:#0d1117;border:1px solid #30363d;color:#e6edf3;border-radius:6px;margin-bottom:12px}.btn{width:100%;padding:10px;background:#58a6ff;border:none;border-radius:6px;font-weight:bold;cursor:pointer;color:#000}.err{background:#3d1a1a;color:#fdaeb7;padding:8px;border-radius:4px;margin-bottom:12px}</style></head><body><div class="box"><h3>Data Hub Pro v3.3</h3>FLASH<form method="POST"><input name="username" placeholder="Username"><input type="password" name="password" placeholder="Password"><button class="btn">Login</button></form><p style="font-size:12px;color:#8b949e;margin-top:12px;">admin / ваш пароль</p></div></body></html>"""
...
def index():
    ...
    tabs = ''.join(['<a href="/datahub/table/' + t[0] + '">' + t[0] + '</a>' for t in tables])
...
def view_table(tablename):
    ...
    for i, row in enumerate(rows):
        ...
        rows_html += '<td><button class="btn btn-green btn-sm" onclick="openEditModal(\'' + str(row[0]) + '\',' + json.dumps(row) + ')">Edit</button> <a href="/datahub/delete-row/' + tablename + '/' + str(row[0]) + '" class="btn btn-red btn-sm" onclick="return confirm(\'Delete?\')">Del</a></td></tr>'
    ...
    return BASE_STYLE + THEME_SCRIPT + hdr(session['user']) + '<div class="tabs">' + tabs + '</div><div class="card"><div class="flex-between"><h3>' + tablename + ' <span class="badge">' + str(total_rows) + ' rows</span></h3><div><button class="btn btn-green btn-sm" onclick="openAddModal()">+ Add</button> <a href="/datahub/export/' + tablename + '?format=csv" class="btn btn-orange btn-sm">CSV</a> <a href="/datahub/export/' + tablename + '?format=xlsx" class="btn btn-orange btn-sm">Excel</a> <a href="/datahub/export/' + tablename + '?format=pdf" class="btn btn-purple btn-sm">PDF</a> <a href="/datahub/copy-table/' + tablename + '" class="btn btn-primary btn-sm">Copy</a> <a href="/datahub/do-rename/' + tablename + '" class="btn btn-primary btn-sm">Rename</a> <a href="/datahub/truncate-table/' + tablename + '" class="btn btn-orange btn-sm" onclick="return confirm(\'Clear?\')">Clear</a> <a href="/datahub/delete-table/' + tablename + '" class="btn btn-red btn-sm" onclick="return confirm(\'Delete?\')">Delete</a></div></div><div class="filter-bar"><input type="text" id="searchInput" placeholder="Search..."><select id="filterColumn">' + chart_cols + '</select><input type="text" id="filterValue"><button class="btn btn-primary btn-sm" onclick="applyFilters()">Apply</button></div><div class="scrollable"><table><tr><th>#</th>' + ''.join(['<th onclick="sortTable(\'' + c + '\')">' + c + '</th>' for c in columns]) + '<th>Actions</th></tr>' + rows_html + '</table></div></div><div class="modal-overlay" id="editModal"><div class="modal"><h3 id="modalTitle">Edit</h3><form method="POST" id="modalForm"><input type="hidden" name="rowid"><div id="modalFields"></div><div class="btn-row"><button type="button" class="btn btn-red btn-sm" onclick="closeModal()">Cancel</button><button type="submit" class="btn btn-green btn-sm">Save</button></div></form></div></div><div class="card"><h3>Quick Stats</h3><div class="row">' + stats_html + '</div></div><div class="card"><h3>Chart</h3><select id="chartType" onchange="buildChart()"><option value="bar">Bar</option><option value="pie">Pie</option></select><select id="chartLabelCol">' + chart_cols + '</select><select id="chartValueCol">' + chart_nums + '</select><input type="color" id="chartColor" value="#58a6ff" onchange="buildChart()"><div class="chart-container"><canvas id="mainChart"></canvas></div></div><div class="card"><h3>Reports</h3><div class="report-grid"><div><h4>Group By</h4><form method="POST" action="/datahub/quick-report/' + tablename + '"><select name="groupcol">' + chart_cols + '</select><select name="aggfunc"><option value="COUNT">COUNT</option><option value="SUM">SUM</option></select><select name="aggcol"><option value="">-- Value --</option>' + chart_nums + '</select><button class="btn btn-green btn-sm" style="width:100%;margin-top:6px;">Run</button></form></div><div><h4>Top N</h4><form method="POST" action="/datahub/top-n/' + tablename + '"><select name="col">' + chart_cols + '</select><input type="number" name="n" value="10"><button class="btn btn-orange btn-sm" style="width:100%;margin-top:6px;">Show</button></form></div><div><h4>Duplicates</h4><form method="POST" action="/datahub/duplicates/' + tablename + '"><select name="col">' + chart_cols + '</select><button class="btn btn-red btn-sm" style="width:100%;margin-top:6px;">Find</button></form></div><div><h4>Null Analysis</h4><form method="POST" action="/datahub/nulls/' + tablename + '"><button class="btn btn-purple btn-sm" style="width:100%;">Analyze</button></form></div></div></div><div class="card"><h3>SQL Console</h3><form method="POST" action="/datahub/sql"><textarea name="query" rows="3"></textarea><button class="btn btn-primary" style="margin-top:8px;">Execute</button></form></div><script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.min.js"> </script><script>const columns=' + json.dumps(columns) + ';const allRows=' + json.dumps([dict(zip(columns, row[1:])) for row in rows]) + ';let mainChart=null;function buildChart(){if(!allRows.length)return;const t=document.getElementById("chartType").value,l=document.getElementById("chartLabelCol").value,v=document.getElementById("chartValueCol").value,c=document.getElementById("chartColor").value;if(!l||!v)return;const g={};allRows.forEach(r=>{const k=String(r[l]||"(empty)"),val=parseFloat(String(r[v]).replace(",","."))||0;g[k]=(g[k]||0)+val});const e=Object.entries(g).sort((a,b)=>b[1]-a[1]).slice(0,20),lbs=e.map(x=>x[0]),d=e.map(x=>x[1]),ctx=document.getElementById("mainChart").getContext("2d");if(mainChart)mainChart.destroy();mainChart=new Chart(ctx,{type:t,data:{labels:lbs,datasets:[{label:v,data:d,backgroundColor:t==="pie"?lbs.map((_,i)=>`hsl(${i*360/lbs.length},70%,50%)`):c+"99",borderColor:c}]},options:{responsive:!0,maintainAspectRatio:!1}})}buildChart();function applyFilters(){const s=document.getElementById("searchInput").value.toLowerCase(),col=document.getElementById("filterColumn").value,val=document.getElementById("filterValue").value.toLowerCase();document.querySelectorAll("table tr:not(:first-child)").forEach(tr=>{let show=!0;if(s)show=tr.textContent.toLowerCase().includes(s);if(show&&col&&val)show=tr.children[columns.indexOf(col)+1]?.textContent.toLowerCase().includes(val);tr.style.display=show?"":"none"})}let sortDir={};function sortTable(col){sortDir[col]=!sortDir[col];const idx=columns.indexOf(col)+1,tbody=document.querySelector("table"),rows=Array.from(tbody.querySelectorAll("tr:not(:first-child)"));rows.sort((a,b)=>{const va=a.children[idx]?.textContent.trim()||"",vb=b.children[idx]?.textContent.trim()||"",na=parseFloat(va),nb=parseFloat(vb);if(!isNaN(na)&&!isNaN(nb))return sortDir[col]?na-nb:nb-na;return sortDir[col]?va.localeCompare(vb):vb.localeCompare(va)});rows.forEach(r=>tbody.appendChild(r))}function openAddModal(){document.getElementById("modalTitle").textContent="Add Row";document.getElementById("modalForm").action="/datahub/add-row/' + tablename + '";let h="";columns.forEach(c=>{h+="<label>"+c+"</label><input type=text name="+c+">"});document.getElementById("modalFields").innerHTML=h;document.getElementById("editModal").classList.add("active")}function openEditModal(id,rd){document.getElementById("modalTitle").textContent="Edit #"+id;document.getElementById("modalForm").action="/datahub/edit-row/' + tablename + '/"+id;let h="";columns.forEach((c,i)=>{h+="<label>"+c+"</label><input type=text name="+c+" value=\\""+String(rd[i+1]||"").replace(/"/g,"&quot;")+"\\">"});document.getElementById("modalFields").innerHTML=h;document.getElementById("editModal").classList.add("active")}function closeModal(){document.getElementById("editModal").classList.remove("active")}</script></div></body></html>'
...
def add_row(tablename):
    ...
    return redirect(url_for('main.view_table', tablename=tablename))
...
def copy_table(tablename):
    ...
    cur.close(); conn.close(); flash('Copied!','success')
    return redirect(url_for('main.view_table', tablename=new_name))
...
def overview():
    if not session.get('user'): return redirect(url_for('main.login_page'))
    conn = get_connection(); cur = conn.cursor()
    cur.execute("SELECT table_name FROM _meta;"); tables = cur.fetchall(); cur.close(); conn.close()
    return BASE_STYLE + THEME_SCRIPT + hdr(session['user']) + '<div class="card"><h3>Overview</h3><p>' + str(len(tables)) + ' tables loaded.</p></div></body></html>'

@blueprint.route('/dashboard')
def dashboard(): return BASE_STYLE + THEME_SCRIPT + hdr(session['user']) + '<div class="card"><h3>Dashboard</h3><p>Coming soon.</p></div></body></html>' if session.get('user') else redirect(url_for('main.login_page'))

@blueprint.route('/pivot', methods=['GET','POST'])
def pivot():
    if not session.get('user'): return redirect(url_for('main.login_page'))
    return BASE_STYLE + THEME_SCRIPT + hdr(session['user']) + '<div class="card"><h3>Pivot</h3><p>Coming soon.</p></div></body></html>'

@blueprint.route('/compare', methods=['GET','POST'])
def compare():
    if not session.get('user'): return redirect(url_for('main.login_page'))
    return BASE_STYLE + THEME_SCRIPT + hdr(session['user']) + '<div class="card"><h3>Compare</h3><p>Coming soon.</p></div></body></html>'

@blueprint.route('/templates')
def templates():
    if not session.get('user'): return redirect(url_for('main.login_page'))
    return BASE_STYLE + THEME_SCRIPT + hdr(session['user']) + '<div class="card"><h3>Templates</h3><p>Coming soon.</p></div></body></html>'

@blueprint.route('/logs')
def logs():
    if not session.get('user'): return redirect(url_for('main.login_page'))
    return BASE_STYLE + THEME_SCRIPT + hdr(session['user']) + '<div class="card"><h3>Logs</h3><p>Coming soon.</p></div></body></html>'
